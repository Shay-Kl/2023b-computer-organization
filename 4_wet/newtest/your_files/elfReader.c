 ...
 */
void offRead(int file, int offset, void* dest, int size)
{
...
unsigned long find_symbol(char* symbol_name, char* exe_file_name, int* error_val)
{
	...
	offRead(file, 0, &header, sizeof(header));
	...
	*error_val = -1;
	...
	offRead(file, sh_off, &section_header, sizeof(section_header));
	int i = 0;
	...
	offRead(file, str_off, &str_table, str_size);
	...
	offRead(file, sym_off, &sym_table, sizeof(sym_table));
